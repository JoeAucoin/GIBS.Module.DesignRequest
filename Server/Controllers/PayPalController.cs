...
namespace GIBS.Module.DesignRequest.Controllers
{
    ...
    public class PayPalController : ModuleControllerBase
    {
        ...
        public PayPalController(IDesignRequestService designRequestService, ILogManager logger, IHttpContextAccessor accessor)
        ...
        public async Task<IActionResult> CreateOrder([FromBody] GIBS.Module.DesignRequest.Models.PaymentRecord paymentRecord)
        {
            try
            {
                // Step 1: Call the service to get the PayPal order response DTO
                var orderResponseDto = await _designRequestService.CreatePayPalOrderAsync(paymentRecord);
        ...
        public async Task<IActionResult> CaptureOrder(string orderId)
        {
            try
            {
                ...
                var resultJson = await _designRequestService.CapturePayPalOrderAsync(orderId, _entityId);
